...
public abstract class Sonda {
    ...
    protected void validarStatusSistema(Coordenada destino, Terreno terreno)
        ...
        double consumoEstimado = distancia * CONSUMO_BASE_MOVIMENTO * terreno.getMultiplicadorConsumo();

        if (bateria.isInsuficiente(consumoEstimado)) {
            throw new BateriaCriticaException(
                String.format("Energia insuficiente para a missao! Necessario: %.1f | Disponivel: %.1f",
                    consumoEstimado, bateria.getCapacidadeAtual()));
        }
